...
# ==============================================================================
# BASE DE DATOS SQLITE - REPOSITORIO VIRTUAL SEGURO
# ==============================================================================
DB_NAME = "repositorio_convivencia.db"


def init_db():
    """Inicializa la base de datos SQLite con validación de esquemas."""
...
def guardar_registro_db(
    ...
    """Guarda un expediente oficial validado estrictamente por un directivo."""
    try:
...
def consultar_registros_db(
    ...
    """Consulta registros asegurando filtros controlados."""
    conn = sqlite3.connect(DB_NAME)
...
# BARRA LATERAL CON CONTROL DE ACCESO SEGURO
with st.sidebar:
    ...
    with st.expander("Ver Marco Legal e Institucional"):
        st.markdown(
            f"* **Constitución Política de 1991** ([Ver PDF]({URL_CONSTITUCION_POLITICA})): Garantiza los derechos fundamentales y el debido proceso."
        )
        st.markdown(
            f"* **Ley 115 de 1994** ([Ver PDF]({URL_LEY_115})): Ley General de Educación."
        )
        st.markdown(
            f"* **Ley 1098 de 2006** ([Ver PDF]({URL_LEY_1098})): Código de Infancia y Adolescencia."
        )
        st.markdown(
            f"* **Ley 1620 de 2013** ([Ver PDF]({URL_LEY_1620})): Sistema Nacional de Convivencia Escolar."
        )
        st.markdown(
            f"* **Manual de Convivencia** ([Ver Documento]({URL_MANUAL_CONVIVENCIA})): Lineamientos institucionales."
        )
...
Tu propósito es orientar automáticamente al usuario en la redacción de documentos institucionales rigurosos, seleccionando el formato adecuado:
1. **ACTA DE COMPROMISO Y CONVIVENCIA ESCOLAR**
2. **MODELO DE CARTA DE DESCARGOS**
3. **REGISTRO EN EL OBSERVADOR DEL ESTUDIANTE**

Aclara que los documentos son borradores previos que deben enviarse a {CORREO_INSTITUCIONAL} para validación directiva oficial.
"""
...
with tab_chat:
    ...
    for idx, msg in enumerate(st.session_state.messages):
        with st.chat_message(msg["role"]):
            ...
            if (
                ...
                and HAS_DOCX
                and any(
                    k in msg["content"].upper()
                    for k in ["ACTA", "CARTA", "REGISTRO"]
                )
            ):
    ...
    if user_input:
        if not api_key:
            st.warning("Ingrese la Clave API de Gemini.")
            st.stop()
        ...
        try:
            ...
            st.error(f"Error en el servicio: {e}")

# PESTAÑA 2: REPOSITORIO VIRTUAL CON SEGURIDAD ESTRICTA
with tab_repositorio:
    st.subheader("Repositorio Digital de Seguimiento Disciplinario")

    if user_role == "Docente / Directivo":
        if autenticado_directivo:
            st.info("Módulo de Publicación Directiva Autorizado.")
            with st.expander("Publicar Documento u Acta Oficial en la Biblioteca"):
                with st.form("form_registro"):
                    ...
                    if st.form_submit_button(
                        ...
                        if f_nombre and f_doc and f_texto:
                            ...
                            st.success(
                                f"Documento de {f_nombre} publicado con éxito."
                            )
                            st.rerun()
            ...
            df_registros = consultar_registros_db(busqueda=search_txt)

            if not df_registros.empty:
                for index, row in df_registros.iterrows():
                    with st.expander(
                        f"Grado {row['grado']} — {row['estudiante_nombre']} ({row['estudiante_documento']})"
                    ):
                        ...
                        if HAS_DOCX:
                            ...
                            st.download_button(
                                ...
                                data=doc_bytes,
                                file_name=f"Doc_{row['estudiante_documento']}.docx",
                                key=f"dl_db_{row['id']}",
                            )
        else:
            st.warning(
        ...
        if doc_estudiante_input:
            ...
            if not df_mis_registros.empty:
                for index, row in df_mis_registros.iterrows():
                    with st.expander(
                        f"Documento Oficial — Grado {row['grado']} ({row['jornada']})"
                    ):
                        st.markdown(row["contenido_texto"])
                        if HAS_DOCX:
                            doc_bytes = generar_documento_word(
                            ...
                            st.download_button(
                                ...
                                data=doc_bytes,
                                file_name=f"Oficial_{row['estudiante_documento']}.docx",
                                key=f"dl_my_db_{row['id']}",
                            )
            else:
                st.success("No se encontraron registros pendientes.")
