...
# ==============================================================================
# CONFIGURACIÓN DE BASE DE DATOS SQLITE (REPOSITORIO INSTITUCIONAL)
# ==============================================================================
...
def guardar_en_db(
    estudiante, documento, grado, asignatura, docente, tipo_falta, contenido
):
  try:
    conn = sqlite3.connect("convivencia_intesac.db")
    cursor = conn.cursor()
    ...
    cursor.execute(
        """
        INSERT INTO registros_convivencia (fecha_creacion, estudiante, documento, grado, asignatura, docente, tipo_falta, contenido_completo)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?)
    """,
        (
            fecha_actual,
...
def consultar_registros_db(filtro_grado="", filtro_busqueda=""):
  ...
  if filtro_busqueda:
    query += (
        " AND (estudiante LIKE ? OR documento LIKE ? OR docente LIKE ? OR"
        " tipo_falta LIKE ?)"
    )
  ...
  if len(st.session_state.messages) <= 1:
    ...
    with col1:
      ...
      if st.button("Conflictos / Agresión verbal o física"):
        selected_option = (
            "Ocurrió una situación de conflicto o agresión entre estudiantes"
      ...
      if st.button("Fraude académico / Plagio"):
        selected_option = (
            "Se reporta una falta relacionada con fraude académico o plagio en"
      ...
      if st.button("Desacato o falta de respeto a docente"):
        selected_option = (
            ...
            " hacia un docente durante el desarrollo de la clase."
        )
    with col2:
      if st.button("Corte de cabello / Uniforme"):
        selected_option = (
            "Se presenta un llamado de atención por corte de cabello inadecuado"
            " o porte incorrecto del uniforme institucional."
        )
      if st.button("Incumplimiento de deberes / Asistencia"):
        selected_option = (
            "Se presentó un incumplimiento en los deberes académicos, faltas de"
            " asistencia o impuntualidad."
        )
      if st.button("Uso no autorizado de celular/equipos"):
        selected_option = (
            "Se reporta el uso no autorizado de teléfono celular o dispositivos"
            " electrónicos durante la jornada escolar."
        )
      if st.button("Evasión de clase / Ausencia en aula"):
        selected_option = (
            "El estudiante ingresó a la institución pero evadió la clase o se"
            " ausentó del aula entre determinadas horas."
        )
      if st.button("Daño a propiedad institucional"):
        selected_option = (
            "Se reportan daños materiales a los pupitres, paredes u otros bienes"
            " de la institución."
        )
...
with tab_repositorio:
  ...
  if registros:
    st.info(f"Se encontraron {len(registros)} registro(s) en el archivo.")
